...
export default function Questions({ currentUser, categoryDetails }) {
  ...
  useEffect(function () {
    ...
    fetch(
      `https://opentdb.com/api.php?amount=${number}&category=${category}&difficulty=${difficulty}&type=multiple&encode=base64`,
    ...
    function () {
      const questionElement = dbData.map((item) => {
        return (
          <Question
            key={item.question}
            list={answers[item.question]}
            {...item}
            number={dbData.indexOf(item)}
          />
        );
      });
  ...
  function checkAnswers() {
    ...
    for (const [key, answer] of Object.entries(answers)) {
      ...
      for (const [no, item] of Object.entries(answer)) {
        ...
        answersList[no] = item.isCorrect
          ? { ...item, correctStyle: "#198754", isChecked: true }
          : item.selected
          ? { ...item, correctStyle: "#dc3545", isChecked: true }
          : { ...item, isChecked: true };
      }
  ...
  function handleSelect(id, name) {
    ...
    for (const [key, answer] of Object.entries(answers)) {
      if (key === name) {
        //apply class of selected for current question only
        const answersList = [];
        for (const [no, item] of Object.entries(answer)) {
          answersList[no] =
            item.key === id
              ? { ...item, selected: true, correctStyle: "#ced4da" }
              : { ...item, selected: false, correctStyle: "" };
        }
        answersElement[key] = answersList;
      } else {
        //the other question remains the same
        ...
        <div className="row">
          ...
          <div className="col-12 col-lg-8">
            ...
            <div className="row">
              {isChecked.checked ? (
                <Link
                  href={`/quiz-list/${category_id}/info`}
                ...
                <button
                  ...
                  className="btn btn-outline-primary btn-icon btn-icon-end sw-25"
                >
